...
<template>
  ...
  <UPopover v-model:open="open" :popper="{ placement: 'bottom' }">
    <UButton
    ...
    />
    <template #panel>
      <UCard :ui="{ body: { base: 'grid grid-cols-5 lg:grid-cols-6 gap-2', padding: 'sm:p-2' } }">
        <UButton
        ...
        >
          <figure :class="f.class" class="w-full rounded overflow-hidden">
            <img :src="base64" :alt="f.label" class="object-cover w-full h-10" />
          </figure>
